set -x
# ============================================================================
# RSO / Search-QA — flat SDAR(技能库特权)  (8 GPU 版)
# ============================================================================
...
#   ②+algorithm.sdar.privileged_source=skill(teacher 的特权 = 本题标准答案,
...
TP=${TP:-2}
OUT=${OUT:-$HOME/rso_runs/search_skill}
...
python3 -m verl.trainer.main_sdar \
    ...
    +algorithm.sdar.privileged_source=skill \
    env.env_name=search \
    env.seed=0 \
    env.max_steps=4 \
    env.rollout.n=8 \
    ...
    trainer.experiment_name=search_skill \
