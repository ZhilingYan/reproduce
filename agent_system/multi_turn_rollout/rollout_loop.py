...
def _gtpriv_from_infos(infos: List[Dict]) -> np.ndarray:
    """Extract per-env ground-truth privileged text from env step/reset infos.

    Unifies TextCraft's 'extra.gt_plan' and ALFWorld's oracle 'extra.expert_plan'
    into a single 'gt_plan' channel used by GT-privileged self-distillation
    (build_teacher_batch, privileged_source='gt'). Returns None-filled array if
    the env exposes neither (e.g. skill-privileged runs), which is harmless.
    """
    return np.array(
        [info.get("extra.gt_plan", info.get("extra.expert_plan")) for info in infos],
        dtype=object,
    )
...
class TrajectoryCollector:
    ...
    def preprocess_single_sample(
        ...
        obs_gtpriv = obs.get('gt_priv', None)
        ...
        # GT-privileged self-distillation: pass the per-episode ground-truth text
        # (TextCraft gt_plan / ALFWorld expert_plan) so build_teacher_batch can use
        # it when privileged_source='gt'. Stored under 'gt_plan' (a default gt_key).
        if obs_gtpriv is not None:
            gtp = obs_gtpriv[item]
            if gtp is not None:
                row_dict['gt_plan'] = gtp if isinstance(gtp, str) else str(gtp)
    ...
    def vanilla_multi_turn_loop(
        ...
        current_gtpriv = _gtpriv_from_infos(infos)
        ...
        for _step in range(self.config.env.max_steps):
            ...
            if current_gamefiles is not None:
                _extra_obs['gamefile'] = current_gamefiles
            if current_gtpriv is not None:
                _extra_obs['gt_priv'] = current_gtpriv
            if _extra_obs:
                obs_input = {**obs, **_extra_obs}
