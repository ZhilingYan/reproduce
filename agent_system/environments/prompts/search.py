...
# [2026-09-19 用户拍板] 推理标签 <think> → <thought>(仅此一处改动,其余 SDAR 原文):
# 模型换 Qwen3-4B-Instruct-2507 后,<think> 是其词表特殊 token(id 151667),
# 探针 22191832 实测该模型对"<think> 里推理"的指令完全无视(直接裸动作),
# <thought>(普通文本 token)则完美服从;SDAR 原配 Qwen2.5 词表无此 token,原文无毒。
# 递归版 search_recursive.py 由本文件派生,自动继承。
...
You should first conduct reasoning process. This process MUST be enclosed within <thought> </thought> tags. 
