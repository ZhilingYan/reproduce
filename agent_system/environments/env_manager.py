...
class AlfWorldEnvironmentManager(EnvironmentManagerBase):
    ...
    def step(self, text_actions: List[str]):
        ...
        # 第 2 件事:128 个环境进程并行执行动作。返回的真实内容比如
        # text_obs[0] = "You arrive at armchair 1. On the armchair 1, you see nothing."
        # 注意此时 rewards 和 dones 还是 Python 元组,比如 (0,) 和 (False,)。
        text_obs, image_obs, rewards, dones, infos = self.envs.step(actions)
        # 第 3 件事,记历史。一个关键细节:存进历史的是 pre_text_obs,也就是模型
        # "做这个决定时看到的旧观测",配上它做的动作。历史记录的含义是
        # (当时看到了什么, 于是做了什么),不是(做了什么, 得到了什么)。
        # 下一行把新观测存起来,作为下一轮的"当时看到了什么"。
...
class TextCraftEnvironmentManager(EnvironmentManagerBase):
    """Environment manager for the TextCraft crafting environment.

    Modeled on WebshopEnvironmentManager. The reset observation contains the
    full recipe listing + goal; we keep that per-env as the "task context" and
    combine it with the latest short step result (e.g. 'Got 3 stick',
    'Crafted 1 wooden pickaxe') plus a SimpleMemory history to build each turn's
    prompt. Privileged GT plan is threaded from info['extra.gt_plan'] untouched.
    """
    ...
    def reset(self, kwargs) -> Dict[str, Any]:
        obs, infos = self.envs.reset()
        # The reset obs is the full "Crafting commands ... Goal: craft X." block.
        self.task_obs = list(obs)          # per-env recipe/goal context
        self.pre_step_obs = ["" for _ in obs]  # latest short step result
        self.pre_text_obs = list(obs)      # what the agent last "saw" (for memory)
    ...
    def step(self, text_actions: List[str]):
        ...
        # store history: what the agent saw last turn + the action it took
        self.memory.store({'text_obs': self.pre_text_obs, 'action': actions})
        # the agent's "observation" for the next turn is the short step result
    ...
    def build_text_obs(self, infos, init: bool = False) -> List[str]:
        """Build the per-env text prompt from the recipe/goal context, the
        latest step result, and the recent action history.
        """
        postprocess_text_obs = []
        if not init and self.config.env.history_length > 0:
            memory_contexts, valid_lens = self.memory.fetch(
        ...
        for i in range(len(self.task_obs)):
            if init:
                current_observation = self.task_obs[i]
            else:
                current_observation = (
                    self.task_obs[i]
    ...
    def _process_batch(self, batch_idx, total_batch_list, total_infos, success):
        for i in reversed(range(len(total_batch_list[batch_idx]))):
            ...
            if batch_item['active_masks']:
                ...
                won_value = float(info['won'])
                success['success_rate'].append(won_value)
                # bucket success by goal recipe depth for finer diagnostics
                depth = info.get('extra.goal_depth')
                if depth is not None:
                    success[f"depth{int(depth)}_success_rate"].append(won_value)
                return


class TextCraftSynthEnvironmentManager(EnvironmentManagerBase):
    """TextCraft-Synth(RAO 官方合成环境的 verl 移植)的字符串加工层。

    与 TextCraftEnvironmentManager 的差异(对应官方设计,见 recursive_OPSD_调试tips.md):
      * 初始观测很短(目标+预算),不含配方表——配方靠 get_info 动作交互查询;
      * 每轮观测 = 任务两行 + 上一步动作的执行结果(成功信息或精确报错);
      * 成功率按难度分桶(easy/medium/hard/extreme),供泛化分析。
    """
    ...
    def reset(self, kwargs) -> Dict[str, Any]:
        obs, infos = self.envs.reset()
        self.task_obs = list(obs)              # 每局的"目标+预算"两行,每轮都带
        self.pre_step_obs = ["" for _ in obs]  # 上一步动作的执行结果
    ...
    def step(self, text_actions: List[str]):
        ...
        # [tips #12] prompt 超长守卫: 超限 = episode 级失败(RAO 哲学: 官方无截断机制,
        # 上下文爆掉就是该 episode 记错误、训练继续),不再是 job 级崩溃(旧配置
        # truncation='error' 曾两次在 140/110 步杀死整个训练)。阈值按字符数近似
        # (26000 字符 ≈ 7K token, 预算 8192 留余量);状态块 prompt 正常 6-12K 字符,
        # 只拦失控增长。触发后观测换成短占位文本,死槽位不再烧 tokenize/生成算力;
        # data.truncation='left' 作为最后保险丝(理论上永不触发)。
    ...
    def build_text_obs(self, infos, init: bool = False) -> List[str]:
        ...
        for i in range(len(self.task_obs)):
            ...
            if init or self.config.env.history_length <= 0:
                obs = TEXTCRAFT_SYNTH_TEMPLATE_NO_HIS.format(
                    current_observation=current_observation)
            else:
                obs = TEXTCRAFT_SYNTH_TEMPLATE.format(
                    current_observation=current_observation,
                    step_count=len(self.memory[i]),
                    history_length=valid_lens[i],
                    action_history=memory_contexts[i],
    ...
    def _process_batch(self, batch_idx, total_batch_list, total_infos, success):
        for i in reversed(range(len(total_batch_list[batch_idx]))):
            ...
            if batch_item['active_masks']:
                ...
                if diff is not None:
                    success[f"{diff}_success_rate"].append(won_value)
                    # easy+medium 合并口径(2026-08-20 用户要求): val100 里这 50 题
                    # 是 flat 方法的"可解区",单独聚合便于观察真实学习进展
                    # (总分被 hard/extreme 的恒 0 稀释)。
                    if diff in ("easy", "medium"):
                        success["easymedium_success_rate"].append(won_value)
                return
...
def make_envs(config):
    ...
    if "search" in config.env.env_name.lower():
        ...
        return envs, val_envs
    elif "textcraft_synth" in config.env.env_name.lower():
        # 注意:这个分支必须排在下面 "textcraft" 分支之前(子串匹配,synth 名字包含 textcraft)
        ...
        env_kwargs = {
            ...
            # val_split="val100" 时验证池换成固定 100 题子集文件(防 OOM,见 envs.py 注释)
            'val_split': getattr(ts_cfg, 'val_split', 'val') if ts_cfg is not None else 'val',
        }
        _envs = build_textcraft_synth_envs(seed=config.env.seed, env_num=config.data.train_batch_size, group_n=group_n, is_train=True, env_kwargs=env_kwargs, resources_per_worker=resources_per_worker)
        _val_envs = build_textcraft_synth_envs(seed=config.env.seed + 1000, env_num=config.data.val_batch_size, group_n=1, is_train=False, env_kwargs=env_kwargs, resources_per_worker=resources_per_worker)

        projection_f = partial(textcraft_synth_projection)
        envs = TextCraftSynthEnvironmentManager(_envs, projection_f, config)
        val_envs = TextCraftSynthEnvironmentManager(_val_envs, projection_f, config)
        return envs, val_envs
        ...
        tc_cfg = getattr(config.env, 'textcraft', None)
        env_kwargs = {
            'max_steps': config.env.max_steps,
            'min_depth': int(getattr(tc_cfg, 'min_depth', 2)) if tc_cfg is not None else 2,
            'test_every': int(getattr(tc_cfg, 'test_every', 5)) if tc_cfg is not None else 5,
        }
        _envs = build_textcraft_envs(seed=config.env.seed, env_num=config.data.train_batch_size, group_n=group_n, is_train=True, env_kwargs=env_kwargs, resources_per_worker=resources_per_worker)
        _val_envs = build_textcraft_envs(seed=config.env.seed + 1000, env_num=config.data.val_batch_size, group_n=1, is_train=False, env_kwargs=env_kwargs, resources_per_worker=resources_per_worker)
