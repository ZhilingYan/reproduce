# -*- coding: utf-8 -*-
"""RSO 阶段 1(纯递归 RL,无蒸馏)的 search 域训练入口。

从 verl/trainer/main_rso.py 克隆(2026-09-20,克隆底本家规:整入口为底,只换必须换的):
  换 1:环境断言 textcraft_synth → search_rso;补 decomp_path 断言
  换 2:环境工厂 → agent_system.environments.env_package.search_rso.opsd_factory.make_rso_search_envs(阶段 1 适配器,无 build_priv,priv 通道不开)
...
def main(config):
    run_rso_search(config)


def run_rso_search(config) -> None:
    ...
    runner = RSOSearchTaskRunner.remote()
    ray.get(runner.run.remote(config))


def validate_rso_search_config(config) -> None:
    ...
    assert "search_rso" in str(config.env.env_name).lower(), (
        f"[RSO/search] 本入口只配 search_rso 环境,收到 env.env_name={config.env.env_name}")
    import os as _os
    dp = str(dict(config.env.get("search_rso", {}) or {}).get("decomp_path", ""))
    assert dp and _os.path.exists(dp), (
        f"[RSO/search] +env.search_rso.decomp_path 缺失或文件不存在:{dp!r}")
    ...
    print(f"[RSO/search] algorithm.rso={rso_alg}  env.rso={rso_env}  "
          "(缺省:progress_coef=0.1, progress_clip=3, baseline_loo=True, invalid_coef=0.1 常开)")


@ray.remote(num_cpus=1)
class RSOSearchTaskRunner:
    def run(self, config):
        ...
        validate_rso_search_config(config)

        local_path = copy_to_local(config.actor_rollout_ref.model.path,
                                   use_shm=config.actor_rollout_ref.model.get("use_shm", False))

        # ---- 换 2:search 域阶段 1 环境工厂 ----
        from agent_system.environments.env_package.search_rso.opsd_factory import (
            make_rso_search_envs)
        envs, val_envs = make_rso_search_envs(config)
