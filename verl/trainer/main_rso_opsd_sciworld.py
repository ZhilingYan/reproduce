# -*- coding: utf-8 -*-
"""RSO + OPSD 的 ScienceWorld 域训练入口。

从 verl/trainer/main_rso_opsd_search.py 克隆(2026-09-19,克隆底本家规:以完整入口为底,
只换 sciworld 域必须换的四样;textcraft / search 入口一个字不动,断言各守各的):
  换 1:环境断言 search_rso → sciworld_rso;去掉 decomp_path 断言(priv 运行时读
        goal_progress,零数据预处理)
  换 2:环境工厂 → sciworld.opsd_factory.make_rso_opsd_sciworld_envs
  换 3:act_tags 断言 = 缺省/["action"](delegate 是 <action> 内菜单动作,单标签)
  换 4:val_batch_size 警戒线按 val 50 case 口径提示
其余(KL 断言、守卫①②、装配、收集器)与底本逐字相同。
方法设计出处:Ideation/ideas/RSO_sciworld_data_mapping.md;
参数出处:Ideation/ideas/参数对照_textcraft_vs_sciworld.md(2026-09-19 拍板)。
"""
import os
...
def main(config):
    run_rso_opsd_sciworld(config)


def run_rso_opsd_sciworld(config) -> None:
    ...
    runner = RSOOPSDSciWorldTaskRunner.remote()
    ray.get(runner.run.remote(config))


def validate_rso_opsd_sciworld_config(config) -> None:
    est = str(config.algorithm.adv_estimator)
    assert est == "rso", f"[RSO+OPSD/sciworld] 优势仍是阶段 1 的 rso,收到 adv_estimator={est!r}"
    assert not bool(config.algorithm.filter_groups.enable), (
        "[RSO+OPSD/sciworld] 不支持 filter_groups(动态采样的多批拼接会破坏收集器的位置回填)")
    assert not bool(config.actor_rollout_ref.actor.get("use_invalid_action_penalty", False)), (
        "[RSO+OPSD/sciworld] 请关闭 actor.use_invalid_action_penalty;无效动作用 +algorithm.rso.invalid_coef 配置")
    assert float(config.actor_rollout_ref.actor.get("entropy_coeff", 0.001)) == 0.0, (
        "[RSO+OPSD/sciworld] 必须 actor.entropy_coeff=0(熵只观测不进梯度)")
    assert bool(config.actor_rollout_ref.actor.get("use_kl_loss", False)), (
        "[RSO+OPSD/sciworld] 必须 actor.use_kl_loss=True(§一点五 底座)")
    kl_coef = float(config.actor_rollout_ref.actor.get("kl_loss_coef", 0.0))
    assert kl_coef == 0.01, f"[RSO+OPSD/sciworld] kl_loss_coef 必须是 0.01,收到 {kl_coef}"
    kl_type = str(config.actor_rollout_ref.actor.get("kl_loss_type", ""))
    assert kl_type == "low_var_kl", f"[RSO+OPSD/sciworld] kl_loss_type 必须是 low_var_kl,收到 {kl_type!r}"
    # ---- 换 1:sciworld 域环境断言(无 decomp_path:priv 运行时读 goal_progress)
    assert "sciworld_rso" in str(config.env.env_name).lower(), (
        f"[RSO+OPSD/sciworld] 本入口只配 sciworld_rso 环境,收到 env.env_name={config.env.env_name}")
    ...
    assert lam > 0, f"[RSO+OPSD/sciworld] lambda_coef 必须大于 0,当前 {lam}"
    assert beta > 0, f"[RSO+OPSD/sciworld] gate_beta 必须大于 0,当前 {beta}"
    tags = list(opsd.get("act_tags", []) or [])
    assert tags in ([], ["action"]), (
        f"[RSO+OPSD/sciworld] act_tags 必须缺省或 [action](delegate/answer: 都在 <action> 内,"
        f"单标签,mapping §2b),收到 {tags}")
    vbs = int(config.data.val_batch_size)
    if vbs > 50:
        print(f"[RSO+OPSD/sciworld] WARNING data.val_batch_size={vbs} > 50:"
              "固定验证 case 只有 50 条(用户拍板 2026-09-19),多出的槽会循环复用")
    rso_alg = dict(config.algorithm.get("rso", {}) or {})
    print(f"[RSO+OPSD/sciworld] algorithm.rso={rso_alg}  rso_opsd={{lambda_coef:{lam}, "
...
def validate_rso_opsd_sciworld_wiring(trainer, config) -> None:
    rso_cfg = dict(config.algorithm.get("rso", {}) or {})
    unknown = set(rso_cfg) - _KNOWN_RSO_KEYS
    assert not unknown, f"[RSO+OPSD/sciworld 接线] algorithm.rso 里有未知键 {sorted(unknown)}"
    for k in _KNOWN_RSO_KEYS:
        if k in rso_cfg:
            cfg_v = rso_cfg[k]
            tr_v = trainer.rso_params[k]
            assert type(tr_v)(cfg_v) == tr_v, (
                f"[RSO+OPSD/sciworld 接线] algorithm.rso.{k}={cfg_v} 没有到达 trainer(读到 {tr_v})")
    opsd_cfg = dict(config.algorithm.get("rso_opsd", {}) or {})
    unknown = set(opsd_cfg) - _KNOWN_OPSD_KEYS
    assert not unknown, f"[RSO+OPSD/sciworld 接线] algorithm.rso_opsd 里有未知键 {sorted(unknown)}"
    ...
    print("[RSO+OPSD/sciworld 接线] 守卫②通过:rso 组 + rso_opsd 组(含 act_tags)+ actor 树全部对上")


@ray.remote(num_cpus=1)
class RSOOPSDSciWorldTaskRunner:
    def run(self, config):
        ...
        validate_rso_opsd_sciworld_config(config)

        local_path = copy_to_local(config.actor_rollout_ref.model.path,
                                   use_shm=config.actor_rollout_ref.model.get("use_shm", False))

        # ---- 换 2:sciworld 域 OPSD 环境工厂 ----
        from agent_system.environments.env_package.sciworld.opsd_factory import (
            make_rso_opsd_sciworld_envs)
        envs, val_envs = make_rso_opsd_sciworld_envs(config)
        ...
        validate_rso_opsd_sciworld_wiring(trainer, config)
