...
def compute_grpo_outcome_advantage(
    ...
    id2score = defaultdict(list)   # 组id → 该组收集到的分数列表
    id2mean = {}
    id2std = {}
    seen_pairs = set()
    with torch.no_grad():
...
def compute_policy_loss(
    ...
    # 逐 token 的概率比值:ratio = exp(log π_new − log π_old) = π_new / π_old
    # (log 空间相减再 exp,比直接除数值稳定)
    negative_approx_kl = log_prob - old_log_prob
    ratio = torch.exp(negative_approx_kl)
    # ppo_kl 只是个监控指标:新旧策略平均偏了多少,不参与损失
    ppo_kl = verl_F.masked_mean(-negative_approx_kl, response_mask)

    # 标准 PPO 裁剪:两个候选损失取较大者(悲观取大)。
    # losses1 = 不裁剪的 −A×ratio;losses2 = ratio 被夹在 [1−0.2, 1+0.2] 后的版本。
    # 效果:策略偏离旧策略太远时梯度被切断,一步不能改太多。
    ...
    # clipfrac = 有多大比例的 token 被裁剪了(监控指标,看更新的"激烈程度")
    pg_clipfrac = verl_F.masked_mean(torch.gt(pg_losses2, pg_losses1).float(), response_mask)

    # dual-clip 补丁(只对优势为负的 token 生效):标准裁剪对"负优势+比值暴涨"的组合
    # 没有保护(−A×ratio 会冲到正无穷),这里再给它封一个上限 −A×3.0,
    # 防止一个坏样本的梯度炸掉整步。
    pg_losses3 = -advantages * clip_ratio_c
    clip_pg_losses2 = torch.min(pg_losses3, clip_pg_losses1)
    pg_clipfrac_lower = verl_F.masked_mean(torch.gt(clip_pg_losses1, pg_losses3) * (advantages < 0).float(), response_mask)

    # 优势为负的 token 用带 dual-clip 的版本,为正的用标准版本;最后聚合成标量
