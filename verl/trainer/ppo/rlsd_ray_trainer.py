...
# Default keys tried (in order) to locate a per-sample ground-truth text when
# privileged_source == "gt": first as top-level env-injected non-tensor columns
# (e.g. TextCraft's 'gt_plan', ALFWorld oracle 'expert_plan'), then as fields
# inside the 'extra_info' / 'reward_model' dicts (e.g. 'solution', 'ground_truth').
DEFAULT_GT_KEYS = ["gt_plan", "expert_plan", "solution", "ground_truth"]

# Wording that explicitly tells the (teacher) model this block is the GROUND-TRUTH
# label, available only as privileged supervision during training. {gt} is filled in.
DEFAULT_GT_PREFIX = (
    "[Ground-Truth Reference]\n"
    "The following is the ground-truth solution/plan for this task. It is provided "
    "only as privileged supervision during training; the student does NOT see it at "
    "test time.\n"
    "{gt}\n\n"
)


def _extract_gt_text(batch: DataProto, i: int, gt_keys):
    """Best-effort per-sample ground-truth text for GT-privileged distillation.

    Returns "" if nothing is found (caller still builds a well-formed prefix).
    """
    ...
    # 1) top-level env-injected / dataset columns
    for k in gt_keys:
        col = ntb.get(k, None)
        if col is not None and i < len(col) and col[i] not in (None, ""):
            return _val(col[i])
    # 2) nested dict columns: extra_info, reward_model
    for container in ("extra_info", "reward_model"):
        col = ntb.get(container, None)
        if col is not None and i < len(col) and isinstance(col[i], dict):
            d = col[i]
            for k in gt_keys:
...
def build_teacher_batch(
    ...
    privileged_source: str = "skill",
    gt_keys=None,
    gt_prefix_template: str = None,
):
    """
    ...
    gt_keys = gt_keys or DEFAULT_GT_KEYS
    gt_prefix_template = gt_prefix_template or DEFAULT_GT_PREFIX
    _gt_miss = 0
    ...
    for i in range(bs):
        ...
        # Build the privileged prefix. Two switchable sources:
        #   privileged_source="skill" (default, original SDAR): skill markdown
        #   privileged_source="gt"   : ground-truth label/plan for this task,
        #                              wrapped in a prefix that says it is the GT.
        if privileged_source == "gt":
            gt_text = _extract_gt_text(batch, i, gt_keys)
            if gt_text == "":
                _gt_miss += 1
            priv_prefix = gt_prefix_template.format(gt=gt_text)
        else:
            ...
            if gamefile is not None:
                ...
                skill_text = skill_provider.get_privileged_info_from_prompt(prompt_text)
            priv_prefix = f"[Privileged Skill Information]\n{skill_text}\n\n"

        # Construct teacher prompt: prepend the privileged info before the prompt
    ...
    if privileged_source == "gt" and _gt_miss > 0:
        print(f"[privileged=gt] WARNING: {_gt_miss}/{bs} samples had no ground-truth "
              f"text found under keys {gt_keys} (empty GT prefix used for those).")
...
class RLSDRayTrainer(RayPPOTrainer):
    ...
    def __init__(self, *args, skill_provider: SkillProvider = None, **kwargs):
        ...
        # Privileged-knowledge source (switchable): "skill" (default, original SDAR)
        # or "gt" (ground-truth label/plan). Read from algorithm.sdar or algorithm.rlsd.
        _sdar_cfg = self.config.algorithm.get("sdar", {}) or {}
        _pk_cfg = _sdar_cfg if _sdar_cfg.get("privileged_source") is not None else rlsd_cfg
        self.privileged_source = (_pk_cfg.get("privileged_source", "skill") or "skill")
        _gt_keys = _pk_cfg.get("gt_keys", None)
        self.gt_keys = list(_gt_keys) if _gt_keys is not None else None
        self.gt_prefix_template = _pk_cfg.get("gt_prefix_template", None)
        print(f"[privileged] source={self.privileged_source} gt_keys={self.gt_keys or 'default'}")
    ...
    def _compute_teacher_log_probs(self, batch: DataProto) -> torch.Tensor:
        ...
        teacher_batch = build_teacher_batch(
            ...
            privileged_source=getattr(self, "privileged_source", "skill"),
            gt_keys=getattr(self, "gt_keys", None),
            gt_prefix_template=getattr(self, "gt_prefix_template", None),
        )
